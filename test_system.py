...
def test_session_logging():
    ...
    try:
        ...
        print("✅ Chart query logged successfully")
        
        # Get session stats
        stats = logger.get_session_stats()
        print(f"✅ Session stats: {stats['total_queries']} queries, {stats['total_tokens']} tokens")
...
def test_llm_tracking():
    ...
    try:
        ...
        print(f"✅ Direct token counting: '{test_text}' = {tokens} tokens")
        
        # Test LLM call tracker
        tracker = LLMCallTracker()
        print("✅ LLM call tracker initialized")
        ...
        print(f"✅ Mock LLM tracking: {total_calls} calls, {total_tokens} tokens")
        
        # Test global tracker
        global_tracker = get_global_tracker()
        stats = global_tracker.get_session_stats()
        print(f"✅ Global tracker ready: {stats['total_calls']} calls tracked")
        
        return True
        
    except Exception as e:
        print(f"❌ LLM tracking test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def test_api_components():
    ...
    try:
        ...
        print(f"✅ FastAPI app ready: {len(routes)} routes configured")
...
def test_memory_system():
    ...
    try:
        ...
        print(f"✅ Memory initialized: max_turns={memory.max_turns}")
        
        # Test basic functionality (without OpenAI API)
        print("✅ Memory structure ready for conversation tracking")
...
def test_streamlit_interface():
    ...
    try:
        ...
        print(f"✅ Streamlit available: version {st.__version__}")
